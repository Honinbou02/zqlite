...
const NextGenDatabase = struct {
    ...
    pub fn storeAIAgent(self: *Self, name: []const u8, config: []const u8, api_key: []const u8) !void {
        ...
        var encrypted_config = try self.crypto.encryptField(config);
        defer encrypted_config.deinit(self.allocator);

        var encrypted_api_key = try self.crypto.encryptField(api_key);
        defer encrypted_api_key.deinit(self.allocator);
    ...
    pub fn cryptoDemo(self: *Self) !void {
        ...
        var hash = try self.crypto.hashPassword(password);
